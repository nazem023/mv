...
const youtube = new YouTube("AIzaSyDF1OvCLpTxiTBsaS-yL1gMA-bPfQd014E"); //تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  ...
  .addField("**support server** :  ", `» <https://discord.gg/wWaH7R4>`, true)
     .addField("**Bot Owner** :  ", `» <@734335509308112896>`, true)
...
\`${prefix}avt\` :Displays you photographed anyone via ID
\`${prefix}avatar\` : To display your picture or a picture you are mention mention
...
\`${prefix}Link\` : Link / Type a link to the chat that brings you a special server link
\`${prefix}invite\` : invite bot your server

\`adminscommands\`:
\`${prefix}clear\` :To delete messages
...
\`${prefix}ticket\` : Open the ticket
\`${prefix}closet\` : To delete a ticket
\`${prefix}say\` : The bot keeps repeating your words
\`${prefix}move\` : To drag the User  to your channel
\`${prefix}reply\` : To create an automatic Reply message
\`${prefix}setLog\` :To select the log channel 
\`${prefix}setby\` : select the exit channel
\`${prefix}setWelcomer <channel name>\` : To select the welcome channel 
\`${prefix}setMessage\` :Select the welcome message
\`${prefix}setVc\` <channel name> : To select a channel Voice Online
\`${prefix}vc off\` : To close the online voice channel
\`${prefix}botsinfo\` : To show all server bots
\`${prefix}role\` : To give User Rol
\`${prefix}role all\` : To give everyone  rol
...
\`credit commands\` : 
\`${prefix}credit\` :Know your balance
\`${prefix}daily\` :To take daily
\`Person to person transfer + credits can only be added to a Daily 



\`music command \`:
\`${prefix}Play\` : Play the song, add it to the list, or complete the song[p]
\`${prefix}Pause\` : Pause the song
\`${prefix}Resume\` : Resume the song 
\`${prefix}stop\` : To stop the song and the bot to exit the channel
\`${prefix}forceskip\` : To directly skip the song
\`${prefix}Queue\` : Show menu 
\`${prefix}skipto\` :To skip to the next song in the next music queue
\`${prefix}Skip\` :Skip to next song
\`${prefix}Volume\` :changing the volme [vol] 
\`${prefix}np\` :   Show what's playing now [np]
\`${prefix}repeat\` : Repeat the song
...
\`${prefix}setVc\` <channel name> : To select a channel Voice Online
\`${prefix}vc off\` : To close the online voice channel
\`${prefix}ls\` : To show all server bots
\`${prefix}role\` : To give User Rol
\`${prefix}role all\` : To give everyone  rol
...
\`${prefix}credit\` :Know your balance
\`${prefix}daily\` :To take daily
\`Person to person transfer + credits can only be added to a Daily command
...
var embed = new Discord.RichEmbed()
.setTitle("invite BOT")
.setDescription("https://discord.com/api/oauth2/authorize?client_id=746719099203223582&permissions=8&scope=bot")
.setColor("RANDOM")
message.channel.sendEmbed(embed)
}
})
